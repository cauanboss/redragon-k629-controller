...
program
  ...
  .action((mode: string, opts: { brightness: string; speed: string; color: string; reportId?: string; patchHandshake?: string }) => {
    ...
    const effectMode = (FIRMWARE_EFFECTS as Record<string, number>)[mode.toUpperCase().replace('-', '_')];
    ...
    let frames = fb.buildFirmwareEffectFrame(effectMode, color, b, s);
    ...
    if (opts.reportId) {
      ...
      console.log(chalk.yellow(`   ⚠ Overriding report ID to 0x${rid.toString(16).padStart(2, '0')}`));
    }
    ...
    if (opts.patchHandshake) {
      ...
      console.log(chalk.yellow(`   ⚠ Patched handshake: ${patch.subarray(0, 8).toString('hex')}`));
    }
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      try {
        ...
        console.log(chalk.green(`   [${label}] ${f.length}B sent — header: ${headerHex}${extra}`));
      } catch (err: unknown) {
    ...
    console.log(chalk.dim('  If nothing changed, firmware bytecodes may be wrong for this model.'));
    console.log(chalk.dim('  Try: pnpm dev:cli debug-effect static  → should show solid color'));
    console.log(chalk.dim('       pnpm dev:cli debug-effect rainbow → should show cycling rainbow'));
