...
const LAYOUT: RowDef[] = [
  ...
  // Row 1 — Number row + Home
  {
    keys: [
      ...
      { id: 'equals', label: '=', width: 1 },
      { id: 'backspace', label: 'Bksp', width: 2 }, null,
  ...
  // Row 2 — QWERTY row + PgUp
  {
    keys: [
      ...
      { id: 'lbracket', label: '[', width: 1 },
      { id: 'rbracket', label: ']', width: 1 },
      { id: 'backslash', label: '\\', width: 1 }, null,
...
function handleServerMessage(raw: unknown): void {
  ...
  switch (msg.type) {
    ...
    case 'profile_list': {
      ...
      for (const name of listMsg.profiles) {
      ...
      for (const row of LAYOUT) {
        for (const item of row.keys) {
          if (item !== null) {
            ...
            colors[item.id] = scaled;  // each key gets the same scaled color
          }
        }
      }
      sendMessage({ type: 'set_colors', colors });
